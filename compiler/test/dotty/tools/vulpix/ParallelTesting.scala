...
import dotc.{Compiler, Driver, FromTasty}
...
trait ParallelTesting extends RunnerOrchestration { self =>
  ...
  private abstract class Test(testSources: List[TestSource], times: Int, threadLimit: Option[Int], suppressAllOutput: Boolean)(implicit val summaryReport: SummaryReporting) { test =>
    ...
    protected def compile(files0: Array[JFile], flags0: TestFlags, suppressErrors: Boolean, targetDir: JFile): TestReporter = {
      ...
      println(flags.isLinkTest + "\n" + files0.toList + "\n\n")
      val files: Array[JFile] =
        if (flags0.isLinkTest) Array.empty
        else files0.flatMap(flattenFiles)
      ...
      val driver =
        if (flags.isLinkTest) FromTasty
        else if (times == 1) new Driver
        else new Driver {
      ...
      try {
        val a =
          if (flags.isLinkTest) Array("Test")
          else files.map(_.getAbsolutePath)

        driver.process(allArgs ++ a, reporter = reporter)
