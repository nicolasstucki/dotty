...
class Definitions {
   ...
   *      }
   */
  def newFunctionNTrait(name: TypeName, lattice: Symbol = ScalaPackageClass) = {
    val completer = new LazyType {
      def complete(denot: SymDenotation)(implicit ctx: Context): Unit = {
        val cls = denot.asClass.classSymbol
        val decls = newScope
  ...
  def isSyntheticFunctionClass(cls: Symbol) =
    scalaClassName(cls).isSyntheticFunction || phantomClassName(cls).isFunction
...
//    valueTypeNameToJavaType(vcls.name) = jtype
  ...
  lazy val PhantomClass: ClassSymbol = {
    ...
    // TODO: Synthesize on demand
    for (arity <- 1 to MaxImplementedFunctionArity) {
      cls.enter(defn.newFunctionNTrait((str.Function + arity).toTypeName, cls))
      cls.enter(defn.newFunctionNTrait((str.ImplicitFunction + arity).toTypeName, cls))
    }
