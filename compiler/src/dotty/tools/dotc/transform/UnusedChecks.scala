...
class UnusedChecks extends MiniPhase {
  ...
  override def transformDefDef(tree: DefDef)(implicit ctx: Context): Tree = checkedValOrDefDef(tree)
  override def transformValDef(tree: ValDef)(implicit ctx: Context): Tree = checkedValOrDefDef(tree)
